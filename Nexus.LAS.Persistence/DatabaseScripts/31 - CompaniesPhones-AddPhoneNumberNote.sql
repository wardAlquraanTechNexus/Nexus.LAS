-- Add PhoneNumberNote column to CompaniesPhones table
...
BEGIN TRY
    PRINT 'Starting CompaniesPhones schema update - Adding PhoneNumberNote column...';

    -- Check if column already exists
    IF NOT EXISTS (
        SELECT 1
        FROM INFORMATION_SCHEMA.COLUMNS
        WHERE TABLE_NAME = 'CompaniesPhones'
        AND COLUMN_NAME = 'PhoneNumberNote'
        AND TABLE_SCHEMA = 'dbo'
    )
    BEGIN
        -- Add PhoneNumberNote column
        ALTER TABLE [NEXUS-RG-LAS-Production-DB].[dbo].[CompaniesPhones]
